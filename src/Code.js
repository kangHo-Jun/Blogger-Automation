...
function isValidOpenAiImage1Url_(url) {
  var normalizedUrl = String(url || '').trim().toLowerCase().split(/[?#]/)[0];
  if (!normalizedUrl || /\.svg$/.test(normalizedUrl)) return false;
  return /\.(png|jpe?g|webp)$/.test(normalizedUrl);
}
...
function processAutoImage_(finalData, imageFolder) {
  ...
  if (!isValidOpenAiImage1Url_(storedOpenAiUrl)) {
    Logger.log('❌ M열 image1_url 검증 실패: OpenAI 실사진 URL이 아닙니다. ' + storedOpenAiUrl);
    throw new Error('M열 image1_url이 비어 있습니다. 메뉴에서 "🤖 OpenAI 이미지 생성"을 먼저 실행하세요.');
  }
  generatedImage1 = {
...
function createV7HTMLPrompt(preprocessData, seoKeywords, highlightKeywords, templateData, styleData, geminiContext) {
  ...
  var system = '[역할]\n' +
    ...
    '- 데이터 없으면 수치 생략\n' +
    '- 수치/사실 뒤에 출처 근거를 괄호로 명시: (예: KS F 기준, 제조사 카탈로그, 현장 시공 사례 기반)\n' +
    '- 출처 불명확한 경우 "업계 일반 기준" 또는 "시공 경험 기반"으로 표기\n\n' +
    '[브랜드 노출 원칙]\n' +
...
function buildBloggerImageToken_(url, description, imageType) {
  return '[[IMG::' + encodeURIComponent(String(url || '')) + '::' +
    encodeURIComponent(String(description || '관련 이미지')) + '::' +
    encodeURIComponent(String(imageType || 'photo')) + ']]';
}
...
function parseBloggerImageToken_(line) {
  ...
  return {
    ...
    imageType: match[3] ? decodeURIComponent(match[3]) : 'photo'
  };
}
...
function mapGeneratedImageUrlsToPlaceholders_(docContent, generatedImages) {
  ...
  for (var i = 0; i < uniqueHolders.length; i++) {
    ...
    var photoDescription = extractPhotoDescriptionFromHolder_(holder);
    var imageType = generated && (generated.mimeType === 'image/svg+xml' || /\.svg(?:[?#]|$)/i.test(publicUrl))
      ? 'svg'
      : 'photo';
    ...
    var escapedHolder = holder.replace(/[\[\]]/g, "\\$&");
    var regex = new RegExp(escapedHolder, "g");
    resultBody = resultBody.replace(regex, buildBloggerImageToken_(publicUrl, photoDescription, imageType));
    mappedCount++;
  }
...
function convertToBloggerHTML(docContent, title, seoKeywords, relatedPosts, contentType) {
  ...
  var buildImageHtml = function (url, description, imageType) {
    var safeDescription = escapeHtml(description || '관련 이미지');
    var captionHtml;
    if (imageType === 'svg') {
      var svgDescription = String(description || '관련 이미지')
        .replace(/\s*비교\s*사진\s*$/g, ' 비교표')
        .replace(/\s*사진\s*$/g, '')
        .trim();
      if (!/(비교표|다이어그램|체크리스트|매트릭스|흐름도)$/.test(svgDescription)) {
        svgDescription += ' 비교표';
      }
      safeDescription = escapeHtml(svgDescription);
      captionHtml = '▲ ' + safeDescription;
    } else {
      captionHtml = '▲ ' + safeDescription + ' (출처: (주)대산 기술팀)';
    }
    return "<div style=\"width:100%; margin:2em 0;\">\n" +
      ...
      "  <p style=\"text-align:center; color:#888; font-size:0.85em; margin-top:0.5em;\">" + captionHtml + "</p>\n" +
      "</div>\n";
  };
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    // 소제목 (###) -> H3
      ...
      html += buildImageHtml(tokenData.url, tokenData.description, tokenData.imageType);
      tagCount.img++;
      tagCount.div++;
    }
    else if (isRawImageUrl(line)) {
      html += buildImageHtml(line, '관련 이미지', /\.svg(?:[?#]|$)/i.test(line) ? 'svg' : 'photo');
      tagCount.img++;
      tagCount.div++;
    }
    else if (/^\*\*[^*]+:\*\*/.test(line)) {
...
function loadMaterialCautions_() {
  ...
  var cached = cache.get('blogger_material_cautions');
  if (cached) return cached;

  try {
    var folderId = '1YhX-ubbEe6sFKPvh-in8dBm_y2SOrfTj';
    var folder = DriveApp.getFolderById(folderId);
    var iter = folder.getFilesByName('material-cautions.md');
    if (!iter.hasNext()) {
      Logger.log('⚠️ material-cautions.md 파일 없음 — 기본값 사용');
      return '';
    }
    ...
    cache.put('blogger_material_cautions', result, 21600);
    Logger.log('✅ material-cautions.md 로드 완료');
    return result;
  } catch (e) {
